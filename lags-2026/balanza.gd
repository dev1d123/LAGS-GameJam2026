...
func _ready():
	# Generar el componente de audio dinámicamente
...
func _actualizar_pantalla_digital():
	var nuevo_texto = str("%.2f" % peso_real) + " KG"
	if label_pantalla.text != nuevo_texto:
		label_pantalla.text = nuevo_texto
		if audio_tick and peso_real > 0.0:
			audio_tick.play()
